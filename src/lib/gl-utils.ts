...
interface FrameBufferOptions {
  /** Trilinear filtering with a mipmap chain: the refraction rim compresses
   *  several texels per screen pixel and aliases without it. Mip levels are
   *  regenerated by the renderer after each scene pass. */
  mipmapped?: boolean;
}
...
export function createFrameBuffer(
  ...
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER,
    options.mipmapped ? gl.LINEAR_MIPMAP_LINEAR : gl.LINEAR);
...
/** Load an HTMLImageElement into a WebGL texture with a full mipmap chain.
 *  The edge refraction compresses several texture texels per screen pixel at
 *  the rim; without mipmaps that undersampling aliases into streaky/uneven
 *  edge patterns. Trilinear filtering band-limits it away. */
